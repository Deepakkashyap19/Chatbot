...
const Message = async (req, res) => {
  try {
    ...
    // Try to save messages to database, but don't fail if database is unavailable
    let userMessage, botMessage;
    
    try {
      userMessage = await User.create({
        sender: "user",
        text: text.trim(),
      });
    } catch (dbError) {
      console.log("Database error saving user message:", dbError.message);
      userMessage = { text: text.trim() };
    }

    try {
      botMessage = await Bot.create({
        sender: "bot",
        text: botResponse,
      });
    } catch (dbError) {
      console.log("Database error saving bot message:", dbError.message);
